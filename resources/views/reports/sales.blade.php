...
<div class="box"><div class="box-body overflow-auto"><table class="table min-w-full whitespace-nowrap table-bordered"><thead><tr><th>Sale No.</th><th>Product</th><th>Customer</th><th>Type</th><th>Quantity</th><th>Unit Price</th><th>Total</th><th>Payment</th><th>Date</th><th>Recorded By</th></tr></thead><tbody>
@forelse($sales as $sale)
<tr><td>{{ $sale->sale_number }}</td><td>{{ $sale->product->product_name ?? '-' }}</td><td>{{ $sale->customer->customer_name ?? 'Walk-in' }}</td><td>{{ ucfirst($sale->sale_type) }}</td><td>{{ number_format($sale->quantity, 2) }}</td><td>{{ number_format($sale->unit_price, 2) }}</td><td>{{ number_format($sale->total_amount, 2) }}</td><td>{{ ucfirst($sale->payment_status) }}</td><td>{{ $sale->sale_date?->format('M d, Y h:i A') }}</td><td>{{ $sale->user->name ?? '-' }}</td></tr>
@empty <tr><td colspan="10" class="text-center">No records found.</td></tr>
@endforelse
